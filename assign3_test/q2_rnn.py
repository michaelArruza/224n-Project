...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(selectedEmbeddings, (-1, self.max_length, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable('b2', shape=(self.config.n_classes,), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        after_softmax = tf.nn.sparse_softmax_cross_entropy_with_logits(preds, self.labels_placeholder)
        masked = tf.boolean_mask(after_softmax, self.mask_placeholder)
