...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        total_window = [[start[i] for i in range(len(start))] for x in range(window_size)]
        total_window += ([[sentence[x][i] for i in range(len(sentence[x]))] for x in range(len(sentence))])
        total_window += ([[end[i] for i in range(len(end))] for x in range(window_size)])
        startIndex = 0
        endIndex = window_size * 2 + 1
        for i in range(len(sentence)):
            windowPt = total_window[startIndex:endIndex]
            flattened = [item for sublist in windowPt for item in sublist]
            packaged = (flattened, labels[i],)
            windowed_data.append(packaged)
            startIndex += 1
            endIndex += 1
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(selectedEmbeddings, (-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
