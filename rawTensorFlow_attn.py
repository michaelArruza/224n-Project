...
class Model():
    ...
    def create_feed_dict(self,inputs_batch, dropout, labels):
        feed_dict = {
    ...
    def add_prediction_op(self):
        ...
        W_c = tf.get_variable('W_c', (self.hidden_size, self.hidden_size + self.max_input_length), tf.float32, tf.contrib.layers.xavier_initializer())
        encoder_hidden_states = [h[1]]
        #print h[1]
        DecoderAttnCell = tf.nn.rnn_cell.LSTMCell(self.hidden_size)
        ...
        for time_step in range(self.max_output_length):
            ...
            for encoder_step in range(self.max_input_length + 1):
                prod = tf.matmul(W_a, tf.transpose(encoder_hidden_states[encoder_step]))
                #print decoderH[1]
                #print prod
                score = tf.matmul(decoderH[1], prod)
                scores.append(score)
                #print score
            #o_drop_t = tf.nn.dropout(o_t, dropout_rate)
            attn = tf.nn.softmax(scores)
            #print attn
            c_t = tf.matmul(attn, encoder_hidden_states)
            #print c_t
            #print decoderH[1]
            h_tilde_t = tf.matmul(W_c, tf.concat([c_t, decoderH[1]], 0))
            final_o_t, final_state = DecoderAttnCell(inputs, h_tilde_t)
            o_drop_t = tf.nn.dropout(final_o_t, dropout_rate)
