...
class Model():
    def __init__(max_output_length,max_input_length,embed_size):
        self.max_output_length = max_output_length
        self.max_input_length = max_input_length
        self.embed_size = embed_size

    def create_feed_dict(inputs_batch, mask_batch, droput, labels):
        self.input_placeholder = tf.placeholder(tf.int32, (None, self._max_input_length, self.n_features))
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable('U', (cell.state_size, self.n_classes), tf.float32, tf.contrib.layers.xavier_initializer())
        b_2 = tf.get_variable('b_2', (self.n_classes,), tf.float32, tf.constant_initializer(0.0))
        h = tf.zeros((1,cell.state_size), tf.float32)
...
def main():
    X = []
    Y = []
    model = Model(15,15,300)
    feed = model.create_feed_dict()
    preds = model.add_prediction_op()
    loss = model.add_loss_op()
    training_op = model.add_training_op()
